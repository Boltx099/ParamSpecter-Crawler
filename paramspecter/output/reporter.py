...
def _save_json(pfx, meta, scanner):
    fname   = f"{pfx}.json"
    # Enrich summary stats for JSON consumers
    oob_hits       = [h for h in scanner.param_hits if h.get("oob")]
    confirmed_hits = [h for h in scanner.param_hits if h.get("confidence", 0) >= 85]
    session_health = (scanner.session_monitor.summary()
                      if getattr(scanner, "session_monitor", None) else None)
    ...
    payload = {
        ...
        "finding_summary": {
            "total_param_hits":  len(scanner.param_hits),
            "oob_blind":         len(oob_hits),
            "confirmed_85plus":  len(confirmed_hits),
            "likely_65plus":     len([h for h in scanner.param_hits if h.get("confidence",0) >= 65]),
        },
    }
    try:
...
def _render_auto_findings(scanner) -> str:
    """Render unified Auto Orchestrator findings section in the HTML report."""
    findings = getattr(scanner, "_auto_findings", None)
    if not findings:
        return ""

    SEV_CLASS = {
        "CRITICAL": "badge-red", "HIGH": "badge-red",
        "MEDIUM": "badge-yellow", "LOW": "badge-blue", "INFO": "badge-gray",
    }
    rows = ""
    for f in sorted(findings, key=lambda x: (
        {"CRITICAL":0,"HIGH":1,"MEDIUM":2,"LOW":3,"INFO":4}.get(x.get("severity","INFO"), 4)
    )):
        sev   = f.get("severity","INFO")
        cls   = SEV_CLASS.get(sev, "badge-gray")
        rows += (
            f"<tr>"
            f"<td><span class='badge {cls}'>{_esc(sev)}</span></td>"
            f"<td>{_esc(f.get('source','?'))}</td>"
            f"<td>{_esc(f.get('category','?'))}</td>"
            f"<td>{_esc(f.get('title','?'))}</td>"
            f"<td style='word-break:break-all'>{_esc(f.get('target','?'))}</td>"
            f"<td style='color:#999;font-size:.8rem'>{_esc(f.get('detail','')[:120])}</td>"
            f"</tr>\n"
        )
    ...
    header = (
        f"<div class='section'>"
        f"<h2>🤖 Auto Orchestrator Findings ({total} total"
        f"{', ' + str(highs) + ' high/critical' if highs else ''})</h2>"
        f"<table><thead>"
        + _th(["Severity","Source","Category","Title","Target","Detail"])
        + f"</thead><tbody>{rows}</tbody></table></div>"
    )
    return header


def _render_ai_triage(scanner) -> str:
    """Render AI triage report section in the HTML report."""
    triage_text = getattr(scanner, "_ai_triage_text", None)
    if not triage_text:
        return ""

    # Convert markdown-ish headings to HTML
    lines_html = []
    for line in triage_text.splitlines():
        stripped = line.strip()
        if stripped.startswith("###"):
            lines_html.append(f"<h4 style='color:#f39c12;margin:12px 0 4px'>{_esc(stripped[3:].strip())}</h4>")
        elif stripped.startswith("##"):
            lines_html.append(f"<h3 style='color:#3498db;margin:14px 0 6px'>{_esc(stripped[2:].strip())}</h3>")
        elif stripped.startswith("#"):
            lines_html.append(f"<h3 style='color:#e74c3c;margin:14px 0 6px'>{_esc(stripped[1:].strip())}</h3>")
        elif stripped.startswith("- ") or stripped.startswith("* "):
            lines_html.append(f"<li style='margin:3px 0 3px 16px;color:#e0e0e0'>{_esc(stripped[2:])}</li>")
        elif stripped == "":
            lines_html.append("<br>")
        else:
            lines_html.append(f"<p style='margin:4px 0;color:#e0e0e0'>{_esc(line)}</p>")

    provider = getattr(scanner, "_ai_provider_name", "AI")
    return (
        f"<div class='section'>"
        f"<h2>🧠 AI Triage Report <span style='font-size:.75rem;color:#888'>({_esc(provider)})</span></h2>"
        f"<div style='line-height:1.7;padding:8px 4px'>{''.join(lines_html)}</div>"
        f"</div>"
    )
...
def _save_html_report(pfx: str, scanner, meta: Dict) -> None:
    ...
    # Separate OOB (blind) from regular param hits for HTML rendering
    reg_hits = [h for h in scanner.param_hits if not h.get("oob")]
    oob_hits = [h for h in scanner.param_hits if h.get("oob")]
    ...
    def _conf_badge(h):
        ...
        if not c:
            return ""
        lbl = h.get("conf_label", "")
        bg  = "#5c1a1a" if c >= 85 else ("#5c4a1a" if c >= 65 else "#2a2a2a")
        tc  = "#ff8080" if c >= 85 else ("#ffd080" if c >= 65 else "#aaa")
        return f"<span class='badge' style='background:{bg};color:{tc}'>{c}% {_esc(lbl)}</span>"

    def _param_hit_rows(hits):
        rows = ""
        for h in hits:
            conf_cell = _conf_badge(h)
            oob_badge = "<span class='badge badge-red'>OOB</span> " if h.get("oob") else ""
            refl_badge= "<span class='badge badge-red'>REFLECTED</span>" if h.get("reflected") else ""
            st = h.get("status", "")
            st_str = str(st) if st else "<span style='color:#e74c3c'>blind</span>"
            rows += (
                f"<tr>"
                f"<td>{_esc(h.get('param',''))}</td>"
                f"<td><code>{_esc(str(h.get('url',''))[:80])}</code></td>"
                f"<td>{st_str}</td>"
                f"<td>{_esc(str(h.get('size_diff','') or ''))}</td>"
                f"<td>{refl_badge}</td>"
                f"<td><code>{_esc(str(h.get('payload',''))[:60])}</code></td>"
                f"<td>{_esc(h.get('cwe',''))}</td>"
                f"<td>{oob_badge}{conf_cell}</td>"
                f"</tr>\n"
            )
        return rows

    param_hits_html = _param_hit_rows(reg_hits)
    oob_hits_html   = _param_hit_rows(oob_hits)
...
<title>ParamSpecter v7.2 Report — {_esc(scanner.start_url)}</title>
<style>
...
<body>
<h1>⚡ ParamSpecter v7.2 — Scan Report</h1>
<p class="meta">
...
<div class="grid">
  ...
  <div class="card"><div class="num" style="color:#e74c3c">{len([h for h in scanner.param_hits if h.get('oob')])}</div><div class="lbl">OOB Blind</div></div>
  <div class="card"><div class="num" style="color:#2ecc71">{len([h for h in scanner.param_hits if h.get('confidence',0)>=85])}</div><div class="lbl">Confirmed 85%+</div></div>
...
{"<div class=\'section\'><h2>🔴 OOB / Blind Findings (" + str(len(oob_hits)) + ") — interactsh confirmed</h2><p style=\'margin-bottom:10px;color:#ff8080;font-size:.9rem\'>These findings were confirmed via out-of-band DNS/HTTP callbacks. DNS does not lie.</p><table><thead>" + _th(["Param","URL","Status","Check","Payload","CWE","Confidence"]) + "</thead><tbody>" + oob_hits_html + "</tbody></table></div>" if oob_hits else ""}

{"<div class=\'section\'><h2>Parameter Hits (" + str(len(reg_hits)) + ")</h2><table><thead>" + _th(["Param","URL","Status","Size Delta","Reflected","Payload","CWE","Confidence"]) + "</thead><tbody>" + param_hits_html + "</tbody></table></div>" if reg_hits else ""}
...
{_render_auto_findings(scanner)}
{_render_ai_triage(scanner)}
